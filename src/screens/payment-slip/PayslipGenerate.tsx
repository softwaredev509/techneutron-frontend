...
import { useGeneratePayslipPdfMutation, useGetPaymentSlipByIdQuery } from '../../services/payment-slip/PaymentSlipsApi';
...
const PayslipForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: FormData) => {
      try {
        const response = await generatePayslipPdf(values.id)
  ...
  const handlePayslipApi = async (Id: number) => {
    setEmployeeId(Id)
    setLoading(true)
  }
        ...
        <Box
          ...
          <Grid container spacing={4}>
            <Grid item xs={19} style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
              ...
              <Button style={{ marginLeft: "1rem", height: "3rem" }} variant="contained" loading={loading} onClick={() => handlePayslipApi(formik?.values?.employee_id)}>Download Data</Button>
            </Grid>
            <Grid item xs={19} style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
              <FormInput>
